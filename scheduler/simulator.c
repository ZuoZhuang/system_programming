...
void print_usage(char *program_name) {
    ...
    fprintf(stderr, "\n");
    fprintf(stderr,
            "Acceptable schemes are: fcfs, sjf, plrtf, pri, ppri, rr#\n");
}
...
int main(int argc, char **argv) {
    ...
    while ((c = getopt(argc, argv, "s:")) != -1) {
        ...
        case 's':
            if (strcasecmp(optarg, "FCFS") == 0) {
                ...
                scheme = PRI;
            } else if (strcasecmp(optarg, "PLRTF") == 0) {
                scheme = PLRTF;
    ...
    } else if (scheme == PLRTF) {
        printf("Preemptive Least Remaining Time First (PLRTF)");
    } else if (scheme == PRI) {
