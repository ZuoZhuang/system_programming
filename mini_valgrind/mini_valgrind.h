...
/*
 * 'total_memory_requested' should store the total memory requested (in bytes)
 * throughout the lifetime of the program, but does not include the overhead
 * for "meta_data".
 *
...
/*
 * 'total_memory_freed' should store the total memory freed (in bytes)
 * throughout the lifetime of the program, but does not include the overhead
 * for "meta_data".
 *
 * Note: This number only increases throughout the lifetime of the program.
...
/*
 * 'invalid_addresses' stores the number of times the user tried free or
 * realloc an invalid address.
 *
 ...
 * @param filename
 *	Name of the file invoking this call to mini_malloc.
 * @param instruction
 *  Address of the instruction invoking this call to mini_malloc.
 *
 * @return
 *	On success, return a pointer to the memory block allocated by the
 *function.
 *	Note that this pointer should return the actual address the block
 *starts,
 *	and not the meta_data.
 *
 *	If the function fails to allocate the requested block of memory, return
 *a
 *	NULL pointer.
 */
...
/*
 * Replace normal calloc.
 *
 * This calloc creates a metadata object and inserts it into the head of the
 * list. You have to allocate enough to hold both the user's requested amount
 * of memory and the meta_data structure. You may call either calloc or malloc
 * in this function, but you should only do it once.
 *
 ...
 * to mini_free. If the pointer is NULL and the size is 0, the behavior is
 * undefined.
 *
 ...
 *	On success, return a pointer to the memory block allocated by the
 *function.
 *	Note that this pointer should return the actual block the pointer
 *starts,
 *	and not the meta_data.
 *
 *	If the function fails to allocate the requested block of memory, return
 ...
 * but increment invalid_addresses instead.
 *
 * @param ptr
