 ...
 * it blocks.
 * If non-negative, the queue will block if the user tries to push when the
 * queue is full.
 ...
 * then you may pass in NULL for the parameters (ex. queue_create(-1, NULL,
 * NULL)).
 ...
 * Note: If thread-safety is off, queue has no max_size.
 */
 ...
 * Note: If thread-safety is off, assumes queue is not empty.
 */
