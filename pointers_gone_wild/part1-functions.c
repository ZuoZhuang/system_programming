...
void two(const char *grade) {
  char *end;
  float grade_f = strtof(grade, &end);
  if (grade_f > 70)
    printf("%f passed!\n", grade_f);
  else
    printf("%s not passed!\n", grade);
...
void eleven(const char *str) {
  char *s = "Hello ";
  char *dest = (char *) malloc(sizeof(char)*strlen(str)+7);
  strcpy(dest, s);
  strcat(dest, str);
  printf("%s\n", dest);
  free(dest);
...
long int clear_bits(long int value, long int flag) {
  // TODO clear_bits
  /*printf("%ld", (long int) (value & (~flag)));*/

  return (long int) (value & (~flag));
}
