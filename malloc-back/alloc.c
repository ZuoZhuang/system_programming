...
void remove_from_list(meta_data_t *list, meta_data_t *block);
...
meta_data_t *get_free_block(meta_data_t **last, size_t size) {
    ...
    if (current && (current->size - size > sizeof(meta_data_t))) {
        /*perror("\n---\n split \n---\n");*/

    fprintf(stderr, "requested: %zu\n", size);
        void *ptr = ((void *)current) + size + META_SIZE;
        meta_data_t *split_block = (meta_data_t *)ptr;

        split_block->size = current->size - size - sizeof(meta_data_t);
        split_block->next = current->next;
        split_block->last = current;
        split_block->free = 0;
        ...
        if (current->next != NULL) current->next->last = split_block;
...
meta_data_t *extend_heap(meta_data_t* last, size_t size) {
    ...
    new_block = (meta_data_t *) sbrk(size + META_SIZE);
    ...
    if (last) {
        ...
        /*if (last->next!=NULL) last->next->last = new_block;*/
        /*last->next = new_block;*/
        new_block->next = last->next;
        last = new_block;
    }
  ...
  // implement malloc!
    ...
    if (!list_head) {
        ...
        if (!new_block) {
            ...
            remove_from_list(free_list, new_block);
        }
    }
...
void add_to_free_list(meta_data_t *block_ptr) {
    ...
    if (free_list == NULL) {
        ...
        block_ptr->next_free = ((meta_data_t *)free_list)->next_free;
        free_list = block_ptr;
    }
}
...
void remove_from_list(meta_data_t *list, meta_data_t *block) {
    ...
    if (list == free_list) {
        ...
        else if (free_list == block) free_list = block->next_free;
        ...
        else if (list_head == block) list_head = block->next;
