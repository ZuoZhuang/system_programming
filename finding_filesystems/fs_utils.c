...
inode_number add_single_indirect_block(file_system *fs_pointer, inode *node) {
    ...
    node->indirect = first_inode;
    node->nlink = 1;
    inode *nody = fs_pointer->inode_root + first_inode;
    nody->nlink = 1;
    nody->mode = TYPE_SINGLE_INDIRECT << RWX_BITS_NUMBER | 0777;
    nody->indirect = UNASSIGNED_NODE;
    int i;
    for (i = 0; i < NUM_DIRECT_INODES; ++i)
        nody->direct[i] = UNASSIGNED_NODE;
    return first_inode;
}
...
inode *find_inode_named(file_system *fs, inode *root, const char *name) {
    ...
    while (size > 0 && root->direct[count] != -1) {
        char *block = (char *)(fs->data_root + root->direct[count]);
        ...
        if (count == NUM_DIRECT_INODES) {
            ...
            root = fs->inode_root + root->indirect;
            count = 0;
        }
    }
    return NULL;
