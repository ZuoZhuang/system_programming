 ...
 *
 * The graph's vertex set is effectively a dictionary which maps targets to
 ...
 * of the `rule_t` type.
 *
 * NOTES ON MEMORY MANAGEMENT / ETC:
 *
 ...
 * any way.
 *
 * Vertices: Adding a string s with `graph_add_vertex(dep_graph, s)`
 ...
 * initialized to NULL by default. If you assign values to the edges, you
 * must manually destroy them before destroying edges. WARNING: if an edge
 * originates from or terminates at a vertex, removing the vertex will
 * also destroy the edge, resulting in a memory leak if you had assigned
 * dynamic memory to that edge.
 *
 *
 * @param makeFileName Path to valid makefile.
 * @param run_targets Null-terminated list of targets as listed on command-line.
 *
 * @return dependency graph representing relevant portion of makefile
 */
